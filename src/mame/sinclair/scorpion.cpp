...
protected:
	...
	virtual void scorpion_io(address_map &map);
...
protected:
	...
	void scorpion_io(address_map &map) override;
...
void scorpion_state::scorpion(machine_config &config)
{
	...
	AY8912(config, m_ay[0], 14_MHz_XTAL / 8)
...
void scorpiontb_state::scorpion_io(address_map &map)
{
	...
	map(0x4021, 0x4021).mirror(0x3fdc)
		.lr8(NAME([this](offs_t offset) { m_maincpu->set_clock_scale(2); return m_port_7ffd_data; }));
	map(0x0021, 0x0021).mirror(0x3fdc)
		.lr8(NAME([this](offs_t offset) { m_maincpu->set_clock_scale(1); return m_port_1ffd_data; }));
...
ROM_START(scorpiontb)
	...
	ROM_SYSTEM_BIOS(13, "scorp_test", "Scorpion Test")
	ROMX_LOAD( "scorp_test.rom", 0x010000, 0x10000, CRC(e0230ca7) SHA1(f38e4d23cb29b4ae3fe8b00a52d7b0f9bb845407), ROM_BIOS(13))
ROM_END
