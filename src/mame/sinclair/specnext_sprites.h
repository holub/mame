...
class specnext_sprites_device : public device_t, public specnext_video_layer_interface
{

public:
	specnext_sprites_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock);

	virtual specnext_video_layer_interface &set_raster_offset(u16 offset_h,  u16 offset_v) override { m_offset_h = offset_h - (OVER_BORDER << 1); m_offset_v = offset_v - OVER_BORDER; update_config(); return *this; }
	virtual specnext_video_layer_interface &set_palette(const char *tag, u16 base_offset, u16 alt_offset) override;

	void update_sprites_cache();
	void draw(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect, u32 pmask);

	virtual void alt_palette_select_w(bool alt_palette_select) override { m_alt_palette_select = alt_palette_select; update_config(); }
