...
template<int HighBits, int Width, int AddrShift> void handler_entry_write_dispatch<HighBits, Width, AddrShift>::range_cut_before(offs_t address, int start)
{
	...
	while(--cut_entry >= 0 && m_u_dispatch[cut_entry]) {
		if(int(LowBits) > -AddrShift && m_u_dispatch[cut_entry]->is_dispatch()) {
			static_cast<handler_entry_write_dispatch<LowBits, Width, AddrShift> *>(m_u_dispatch[cut_entry])->range_cut_before(address);
			break;
		}
...
template<int HighBits, int Width, int AddrShift> void handler_entry_write_dispatch<HighBits, Width, AddrShift>::range_cut_after(offs_t address, int start)
{
	...
	while(++cut_entry < COUNT && m_u_dispatch[cut_entry]) {
		if(int(LowBits) > -AddrShift && m_u_dispatch[cut_entry]->is_dispatch()) {
			static_cast<handler_entry_write_dispatch<LowBits, Width, AddrShift> *>(m_u_dispatch[cut_entry])->range_cut_after(address);
			break;
		}
